...
try {
    ...
    Invoke-RecordedDocker "source-image.json" @("run", "--rm", "--network", "none", "--entrypoint", "python", "$Project-test", "-m", "scripts.source_manifest")
    $manifest = Get-Content -Raw (Join-Path $evidence "source-image.json") | ConvertFrom-Json -AsHashtable
    foreach ($entry in $manifest.GetEnumerator()) {
        $actual = (Get-FileHash -Algorithm SHA256 -LiteralPath (Join-Path $workspace $entry.Key)).Hash.ToLower()
        if ($actual -ne $entry.Value) { throw "Image/source mismatch: $($entry.Key)" }
    }
    "Verified $($manifest.Count) source files against tested image." | Set-Content (Join-Path $evidence "source-check.txt")
...
Get-ChildItem -File -Recurse $evidence | Where-Object Name -ne "sha256sums.txt" | Sort-Object Name | ForEach-Object {
